        ...
        components={{
          code({ node, className, children, ...props }) {
            ...
            if (match && match[1] === 'openscad') {
              return <div className="text-xs italic text-slate-500 my-1">[Code displayed in preview panel]</div>;
            }
            return <code className={`${className} bg-slate-800 px-1 py-0.5 rounded text-xs`} {...props}>{children}</code>;
          }
        }}
      >
        {msg.text}
...
const ChatArea: React.FC<ChatAreaProps> = memo(({ messages, input, setInput, onSend, isLoading }) => {
