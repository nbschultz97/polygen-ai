...
const ScadRenderer: React.FC<ScadRendererProps> = memo(
  ({ code, isProUser, uploadedStlData, onRenderComplete }) => {
    ...
    const compileAndRender = useCallback(
      async (mode: 'preview' | 'full' = renderMode) => {
        ...
        try {
          console.log(
            `[ScadRenderer] compileAndRender(${mode}): code length=${code.length}, starting WASM...`
          );
          ...
          console.log('[ScadRenderer] WASM instance created, FS available:', !!instance?.FS);
          if (!instance?.FS) {
          ...
          console.log(`[ScadRenderer] STL read: ${stlData?.length ?? 0} bytes`);
          ...
          console.log(
            `[ScadRenderer] STL parsed: ${geometry?.attributes?.position?.count ?? 0} vertices`
          );
          ...
          console.error('[ScadRenderer] Render error:', err);
          setError(err?.message || String(err) || 'Failed to render. Please check your code.');
        } finally {
                ...
                {renderStats.dimensions && (
                  <div className="flex justify-between gap-4 text-slate-400">
                    ...
                    <span className="text-cyan-400 font-mono text-[9px]">
                      {renderStats.dimensions.x}×{renderStats.dimensions.y}×
                      {renderStats.dimensions.z}
                      mm
                    </span>
                  </div>
                )}
              ...
              {error.includes('Network error') || error.includes('could not be retrieved') ? (
                <>
                  ...
                  <p className="text-sm text-slate-400 mb-4">
                    The OpenSCAD engine couldn't load from CDN. This is usually a network
                    restriction.
                  </p>
