...
### 2b. THE HULL HEURISTIC (CRITICAL FOR CONNECTIVITY)
When connecting two parts (e.g., a funnel to a tube, a plate to a bracket, any two volumes):
- DO NOT calculate coordinate alignment or trigonometry to match faces
- INSTEAD: Place a primitive (sphere, cylinder) at Point A, another at Point B, and wrap in hull()
- This GUARANTEES a watertight, manifold connection without math errors

EXAMPLES:
// Rounded box (instead of library rcube):
...
// 90-degree elbow (instead of complex rotate_extrude):
// Place cylinders at start/end, sphere at bend point, hull() them
module elbow_joint(d, bend_radius) {
    hull() {
        cylinder(h=0.1, d=d, center=true);  // start
        translate([bend_radius, 0, bend_radius])
            sphere(d=d);  // bend keyframe
    }
    hull() {
        translate([bend_radius, 0, bend_radius])
            sphere(d=d);  // bend keyframe
        translate([bend_radius, 0, 0])
            rotate([0, 90, 0]) cylinder(h=0.1, d=d, center=true);  // end
    }
}

// Bracket/arm connecting two points:
module arm(start, end, thickness) {
    hull() {
        translate(start) sphere(d=thickness);
        translate(end) sphere(d=thickness);
    }
}

RULE: When the GST has "hull" type or any part needs connectivity between two points,
use hull() with keyframe primitives. NEVER manually calculate face alignment.
